...
const constructionConeGlb = await gltfLoader.loadAsync(
  `${import.meta.env.BASE_URL}Day 33 - Construction Cone.glb`
);
const constructionCone = constructionConeGlb.scene;

constructionCone.traverse((child) => {
  if (child.isMesh) {
...
constructionCone.updateMatrixWorld(true);

// compute bounds and scale so the model's largest dimension equals `targetSize`
let box = new THREE.Box3().setFromObject(constructionCone);
const size = box.getSize(new THREE.Vector3());
const maxDim = Math.max(size.x, size.y, size.z);
const targetSize = 4; // world units you want the model to fit in
if (maxDim > 0) {
  const scale = targetSize / maxDim;
  constructionCone.scale.setScalar(scale);
  constructionCone.updateMatrixWorld(true); // update after scaling
}

// recompute bounds and get center
box = new THREE.Box3().setFromObject(constructionCone);
const center = box.getCenter(new THREE.Vector3());

// create a pivot at the world origin and add the model offset so its center is at pivot
const pivot = new THREE.Group();
scene.add(pivot);
constructionCone.position.sub(center); // move model so its center is at (0,0,0) relative to pivot
pivot.add(constructionCone);
